from flask import render_template, redirect, url_for, flash, request, current_app, abort, g
from werkzeug.urls import url_parse
from flask_login import login_user, logout_user, current_user, login_required
from flask_babel import _
from app import db, get_locale, current_app
from app.auth import bp
from app.auth.forms import LoginForm, RegistrationForm, forgetPasswordRequestForm, resetPasswordForm
...
def before_request():
    ...
    if request.endpoint and request.endpoint != 'static' and request.endpoint!="auth.register" and request.endpoint!="auth.login" and request.endpoint!="main.index":
        ...
        if f_1=="2":
            if current_user.is_anonymous or (not current_user.moderator() and not current_user.admin()):
                return render_template("errors/503.html", text=f_2)
        elif f_1=="3":
    ...
    if current_user.is_authenticated:
        ...
        if current_user.banned and request.endpoint and request.endpoint[:5] != 'auth.' and request.endpoint != 'static':
...
def login():
    ...
    return render_template('auth/login.html', title='Вход', form=form, site_key=site_key)
...
def register():
    ...
    if form.validate_on_submit():
        if form.email.data==current_app.config['EMAIL']:
            ...
            role=1
            confirmed=False
        user = User(username=form.username.data, email=form.email.data, confirmed=confirmed, role=role, blogger=confirmed, first_name=form.first_name.data, last_name=form.last_name.data, stroage_granted=5242880)
        user.set_password(form.password.data)
        db.session.add(user)
        db.session.commit()
        token = user.generate_confirmation_token()
        f=open("app/templates/auth/confirm.html")
        confirm_html=f.read().replace("{url}", f"{ url_for('auth.confirm', token=token, _external=True) }").replace("{name}", f"{user.username}")
        f.close()
        send_email(user.email, "Подтвердите свой аккаунт - DigitalBlog", confirm_html)
        flash(_('Вы успешно зарегистрированы!'), "success")
        user.add_notify('Добро пожаловать!', 'Вы успешно зарегестрировались на сайте DigitalBlog!')
        db.session.commit()
        login_user(user, remember=True)
        return redirect(url_for('main.index'))
    return render_template('auth/register.html', title='Регистрация', form=form, text=text, site_key=site_key)
...
def confirm(token):
    try:
        ...
        flash(_('Ссылка для подтверждения повреждена или срок её действия истёк.'), 'danger')
...
def resend_confirmation():
    ...
    f=open("app/templates/auth/confirm.html")
    confirm_html=f.read().replace("{url}", f"{ url_for('auth.confirm', token=token, _external=True) }").replace("{name}", f"{current_user.username}")
    f.close()
    send_email(current_user.email, "Подтвердите свой аккаунт - DigitalBlog", confirm_html)
...
def forget_password_request():
    ...
    if form.validate_on_submit():
        ...
        if user is not None:
            ...
            reset_password_html=f.read().replace("{url}", f"{ url_for('auth.reset_password', token=token, _external=True) }").replace("{name}", f"{user.username}")
            f.close()
            send_email(user.email, "Сброс вашего пароля - DigitalBlog", reset_password_html)
        return render_template('auth/forgetPassswordNotificationMessage.html',email=email)
    return render_template("auth/forgetPasswordRequest.html", form=form, title='Сброс пароля', site_key=site_key)
...
def reset_password(token):
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            flash(_("Ссылка для сброса пароля повреждена или срок её действия истёк."), "danger")
            return redirect(url_for("auth.login"))
        return redirect(url_for('auth.login'))
