...
class User(UserMixin, db.Model):
    ...
    def avatar(self):
        if self.avatar_url == "static/default.png":
            return "https://digitalblog.github.io/cdn/img/default.webp"
        else:
            return url_for("main.avatar", user_id=self.id)

    def view(self, post):
        if (
            not PostView.query.filter(
                PostView.user_id == self.id, PostView.post_id == post.id
            ).count()
            > 0
        ):
    ...
    def unrate_post(self, post):
        rate = PostReactions.query.filter(
            PostReactions.user_id == self.id, PostReactions.post_id == post.id
        )
        if self.is_rated_post(post):
            if rate.t == 1:
                ...
                post.dislikes_count -= 1
            PostReactions.query.filter_by(user_id=self.id, post_id=post.id).delete()
    ...
    def unfavourite_post(self, post):
        if self.is_favourited_post(post):
            post.favourites_count -= 1
            PostFavourites.query.filter_by(user_id=self.id, post_id=post.id).delete()

    def is_favourited_post(self, post):
        return (
            PostFavourites.query.filter(
                PostFavourites.user_id == self.id, PostFavourites.post_id == post.id
            ).count()
            > 0
        )
    ...
    def unlike_comment(self, comment):
        if self.is_liking_comment(comment):
            CommentLikes.query.filter_by(
                user_id=self.id, comment_id=comment.id
            ).delete()

    def is_liking_comment(self, comment):
        return (
            CommentLikes.query.filter(
                CommentLikes.user_id == self.id, CommentLikes.comment_id == comment.id
            ).count()
            > 0
        )
    ...
    def friends(self):
        friends = (
            ...
            .filter(Followers.followed_id == self.id)
            .filter(
                User.id.in_(
    ...
    def new_notifies(self):
        ...
        return (
            Notify.query.filter_by(user=self)
            .filter(Notify.timestamp > last_read_time)
            .count()
        )
...
class Post(db.Model):
    ...
    views = db.relationship(
        "PostView", backref="post", lazy="dynamic", cascade="all,delete"
    )
    ...
    favourites = db.relationship(
        "PostFavourites", backref="post", lazy="dynamic", cascade="all,delete"
    )
...
class Comment(db.Model):
    ...
    likes = db.relationship(
        "CommentLikes", backref="comment", lazy="dynamic", cascade="all,delete"
    )
