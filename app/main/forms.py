...
from wtforms import StringField, SubmitField, TextAreaField, PasswordField, FileField, BooleanField, SelectField, IntegerField
...
class ChangePasswordForm(FlaskForm):
    ...
    password = PasswordField(_l('Новый пароль:'), validators=[DataRequired(), Length(min=6 ,max=128)])
    password2 = PasswordField(_l('Повторите новый пароль:'), validators=[DataRequired(), EqualTo('password', message='Текст этого поля должен совпадать с верхним полем.')])
...
class EditProfileForm(FlaskForm):
    photo = FileField(validators=[FileAllowed(['jpg', 'jpeg', 'png', 'bmp', 'gif', 'tif', 'webp', 'heic', 'avif'], _l('Файл не является изображением'))])
    username = StringField(_l('Имя пользователя:'), validators=[DataRequired(), Length(min=3, max=20), Regexp('^[A-Za-z0-9_]*$', message="a-z,0-9,_")])
    ...
    email = StringField(_l('Email(при изменении вы выйдете из аккаунта и он станет неподтверждённым):'), validators=[DataRequired(), Email(), Length(min=1, max=64)])
    profile_access = SelectField(_l('Кто может просматривать мой профиль:'), coerce=int, choices=[('0', 'Все пользователи'), ('1', 'Подписчики'), ('2', 'Подписки'), ('3', 'Друзья'), ('4', 'Никто')])
    message_setting = SelectField(_l('Кто может отправлять мне сообщения:'), coerce=int, choices=[('0', _l('Все пользователи')), ('1', _l('Подписчики')), ('2', _l('Подписки')), ('3', _l('Друзья')), ('4', _l('Никто'))])
    anonymous_show = BooleanField(_l('Показывать профиль неавторизованным пользователям'))
...
class EditProfileMainAdminForm(FlaskForm):
    photo = FileField()
    role = SelectField('Роль:', coerce=int, choices=[('1', 'Пользователь'), ('2', 'Модератор'), ('3', 'Администартор'), ('4', 'Главный администратор')])
    username = StringField(_l('Имя пользователя:'), validators=[DataRequired(), Length(min=3, max=20), Regexp('^[A-Za-z0-9_]*$', message="a-z,0-9,_")])
    first_name = StringField(_l('Имя:'), validators=[Length(max=24)])
    last_name = StringField(_l('Фамилия:'), validators=[Length(max=24)])
    about_me = TextAreaField(_l('Обо мне:'),  validators=[Length(min=0, max=100)])
    ...
    profile_access = SelectField(_l('Кто может просматривать мой профиль:'), coerce=int, choices=[('0', 'Все пользователи'), ('1', 'Подписчики'), ('2', 'Подписки'), ('3', 'Друзья'), ('4', 'Никто')])
    message_setting = SelectField('Кто может отправлять мне сообщения:', coerce=int, choices=[('0', 'Все пользователи'), ('1', 'Подписчики'), ('2', 'Подписки'), ('3', 'Друзья'), ('4', 'Никто')])
    anonymous_show = BooleanField(_l('Показывать профиль неавторизованным пользователям'))
    granted_stroage = IntegerField('Выделеное хранилище(в байтах):', validators=[DataRequired()])
...
class EditProfileAdminForm(FlaskForm):
    ...
    about_me = TextAreaField(_l('Обо мне:'),  validators=[Length(min=0, max=100)])
    profile_access = SelectField(_l('Кто может просматривать мой профиль:'), coerce=int, choices=[('0', 'Все пользователи'), ('1', 'Подписчики'), ('2', 'Подписки'), ('3', 'Друзья'), ('4', 'Никто')])
    message_setting = SelectField(_l('Кто может отправлять мне сообщения:'), coerce=int, choices=[('0', 'Все пользователи'), ('1', 'Подписчики'), ('2', 'Подписки'), ('3', 'Друзья'), ('4', 'Никто')])
    anonymous_show = BooleanField(_l('Показывать профиль неавторизованным пользователям'))
    granted_stroage = IntegerField(_l('Выделеное хранилище(в байтах):'), validators=[DataRequired()])
        ...
        if file.data:
            ...
            if size> 10*1024*1024:
                ...
                if self.original_stroage_used+size>self.original_stroage_granted:
                    os.remove(f'app/static/{filename}')
                    raise ValidationError(_('Не хватает места, посетите "Использование"'))
                else:
...
class MessageForm(FlaskForm):
    file = FileField()
    title = StringField(_l('Тема сообщения:'), validators=[DataRequired(), Length(min=1, max=70)])
    body = TextAreaField(_l('Содержание сообщения:'), render_kw = {"placeholder":_l("Содержание сообщения")})
    submit = SubmitField(_l('Отправить'))
...
class CommentForm(FlaskForm):
    comment = PageDownField(_l('Комментарий:'), validators=[DataRequired(), Length(min=1, max=350)])
    submit = SubmitField(_l('Отправить'))