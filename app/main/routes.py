...
from flask import render_template, flash, redirect, url_for, request, g, \
    jsonify, current_app, abort, send_file, after_this_request, make_response
from flask_login import current_user, login_required
from flask_babel import get_locale, _
from langdetect import detect, LangDetectException
from app import db
from app.main.forms import EditProfileForm, EmptyForm, PostForm,\
    MessageForm, CommentForm, ChangePasswordForm, EditProfileAdminForm, EditProfileMainAdminForm, SendNotifyForm
from app.models import User, Post, Message, Notification, Comment, CommentLikes, PostView, PostFavourites, Notify, Followers
...
def index():
    if not current_user.is_anonymous:
        ...
        if show_posts=='4':
            ...
            query = Post.query.filter(PostFavourites.user_id == current_user.id,PostFavourites.post_id == Post.id)
        else:
            query = Post.query
        if order=='6':
            order_query=Post.last_update_time
...
def moderation():
    ...
    comments=comments.order_by(Comment.timestamp.desc()).paginate(page=page, per_page=current_app.config['POSTS_PER_PAGE'], error_out=False)
    return render_template('moderation.html', comments=comments, m=markdown.markdown, pagination=comments, title='Модерация')
...
def message_detail(message_id):
    ...
    if current_user==message.recipient and message.read==False:
        ...
        db.session.commit()
    return render_template('detail_message.html', title=message.title, message=message, file=file, read_time_formatted=read_time_formatted)
...
def user(username):
    ...
    if current_user.is_anonymous:
        ...
        if current_user.admin() or current_user==user:
            ...
            abort(403)
        elif user.show_profile_id==3 and not (current_user.is_following(user) and current_user.is_followed_by(user)):
            abort(403)
    ...
    return render_template('user.html',title='Профиль - {}'.format(username), user=user, posts=posts.items, comments=comments.items, form=form, pagination_posts=posts, pagination_comments=comments, m=markdown.markdown, page_posts=page_posts, page_comments=page_comments, len=len, formNotify=formNotify)
...
def delete_comment(comment_id):
    ...
    if comment.author != current_user and not current_user.admin() and not current_user.moderator():
        abort(403)
...
def edit_user_profile(user_id):
    ...
    if form.validate_on_submit():
        ...
        if current_user.main_admin():
        	...
        	form.anonymous_show.data = user.anonymous_show
        	form.profile_access.data = user.show_profile_id
    return render_template('edit_profile.html', title='Редактирование профиля', form=form, user=user)                                        
...
def delete_user(user_id):
    ...
    if current_user.main_admin() or current_user==user:
        if user.main_admin() and not current_user.main_admin():
        	abort(403)
        user.delete_avatar()
        msgs=Message.query.filter((Message.recipient_id == user.id) | (Message.sender_id == user.id)).all()
        for m in msgs:
            try:
                ...
                pass
        Message.query.filter((Message.recipient_id == user.id) | (Message.sender_id == user.id)).delete()
        posts=Post.query
        for post in posts:
...
def users():    
    ...
    if not current_user.is_anonymous:
        ...
        if show_users=='3':
            query=User.query.join(Followers, User.id == Followers.followed_id).filter(Followers.follower_id == current_user.id).order_by(User.last_seen.desc())
        elif show_users=='4':
            query=User.query.join(Followers, User.id == Followers.follower_id).filter(Followers.followed_id == current_user.id).order_by(User.last_seen.desc())
        elif show_users=='5' and current_user.main_admin():
            query=User.query.filter_by(confirmed=False).order_by(User.last_seen.desc())
        elif show_users=='6' and current_user.main_admin():
            query=User.query.filter_by(banned=True).order_by(User.last_seen.desc())
        elif show_users=='2':
            query=User.query.join(Followers, User.id == Followers.follower_id).filter(Followers.followed_id == current_user.id).filter(User.id.in_(User.query.join(Followers, User.id == Followers.followed_id).filter(Followers.follower_id == current_user.id).with_entities(User.id))).order_by(User.last_seen.desc())
        else:
            query=User.query.order_by(User.last_seen.desc())
    else:
        query=User.query.order_by(User.last_seen.desc())
        show_users=1
    users = query.paginate(page=page, per_page=current_app.config['POSTS_PER_PAGE'], error_out=False)
    return render_template('users.html', description="Сообщество для программистов и увлекающихся IT. Подписывайтесь, создавайте публикации, обсуждайте, ставьте лайки, пишите личные сообщения, а также возможность подкреплять файлы к публикациям и сообщениям.",title='Пользователи', users=users.items, datetime=datetime, pagination=users, show_users=show_users, User=User)
...
@login_required
def billing():
    return render_template('billing.html', title="Использование", used_space=current_user.stroage_used, total_space=current_user.stroage_granted, user=current_user)
...
def notifies():
    ...
    ns=ns.order_by(Notify.timestamp.desc()).paginate(page=page, per_page=current_app.config['POSTS_PER_PAGE'], error_out=False)
    @after_this_request
    def update(response):
        current_user.last_notify_read_time = datetime.utcnow()
        db.session.commit()
        return response
    return render_template('notifies.html',title='Уведомления', ns=ns.items, len=len, pagination=ns)
...
def send_message(recipient):
    ...
    if user.message_setting!=0:
        if current_user.admin() or current_user==user:
            ...
            abort(403)
        elif user.message_setting==3 and not (current_user.is_following(user) and current_user.is_followed_by(user)):
            abort(403)
        elif user.message_setting==4:
            abort(403)
    form = MessageForm(current_user.id,current_user.stroage_used,current_user.stroage_granted)
    if form.validate_on_submit():
        time=datetime.utcnow()
        message = Message(author=current_user, recipient=user, body=form.body.data, title=form.title.data, timestamp=time, last_update_time=time)
        f=form.file.data
    ...
    form.body.data = " "
    form.title.data = ""
    return render_template('send_message.html', title=f'Отправить личное сообщение {user.username}'.format(recipient),form=form, recipient=recipient)
...
def messages():
    ...
    page = request.args.get('page', 1, type=int)
    if current_user.main_admin():
        messages=Message.query.order_by(Message.timestamp.desc()).paginate(page=page, per_page=current_app.config['POSTS_PER_PAGE'], error_out=False)
    else:
        messages=Message.query.filter((Message.recipient_id == current_user.id) | (Message.sender_id == current_user.id))
        messages=messages.order_by(Message.timestamp.desc()).paginate(page=page, per_page=current_app.config['POSTS_PER_PAGE'], error_out=False)
    @after_this_request
    def update(response):
        current_user.last_message_read_time = datetime.utcnow()
        db.session.commit()
        return response
    return render_template('messages.html',title='Личные сообщения', messages=messages.items, len=len, pagination=messages)
...
def update_post(post_id):
    ...
    if form.validate_on_submit():
        ...
        if f:
            ...
            try:
                ...
                new_stroage_used=int(post.author.stroage_used)-int(post.file_size)+int(f[1])
                post.file_size=f[1]
    ...
    return render_template('create_post.html', title='Редактирование публикации', form=form, file=file, name=name, post=post)
...
def update_message(message_id):
    ...
    if form.validate_on_submit():
        ...
        if f:
            ...
            try:
                ...
                os.rename(f[0], f'app/uploads/message-{message.id}/{fn}')
                new_stroage_used=int(message.author.stroage_used)-int(message.file_size)+int(f[1])
                message.file_size=f[1]
    ...
    return render_template('update_message.html', title='Редактирование сообщения', form=form, name=name, file=file, message=message)
...
def update_comment(comment_id):
    ...
    return render_template('update_comment.html', title='Редактирование комментария', form=form, comment=comment)
